...
describe('Atom', () => {
  testEq(
    `{const x = Atom({x: 2}); x.view("x").modify(R.inc) ; return x.get()}`,
    { x: 3 }
  )
...
describe('view', () => {
  ...
  testEq(
    `{const xy = Atom({x: {y: 1}}), y = xy.view("x");
      y.set({y: 3});
      return y}`,
    { y: 3 }
  )
  testEq(
    `{const xy = Atom({x: {y: 2}}), y = xy.view("x"), z = y.view("y");
           return z}`,
    2
  )
  testEq(
    `{const xy = Atom({x: {y: 3}}), z = xy.view(["x", "y"]);
      z.set(2);
      return z}`,
    2
  )
